...
function updateNowPlaying(){
	var callback = function(parsedInfo) {
		...
		chrome.runtime.sendMessage({type: 'validate', artist: artist, track: track, album: album}, function(response) {
		  if (response != false) {
			  chrome.extension.sendMessage({type: 'nowPlaying', artist: artist, track: track, album: album, source: 'Napster'});
		  }
...
function scrobbleTrack() {
   // stats
   chrome.runtime.sendMessage({type: 'trackStats', text: 'The Napster song scrobbled'});
