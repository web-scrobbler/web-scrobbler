...
let _albumData = $.extend({}, defaultAlbumData);
...
Connector.getAlbum = () => {
	...
	if (!isAlbumDataLoaded(_albumData)) {
		_albumData.loaded = loadAlbumData((album_name) => _albumData.name = album_name);
	}
...
// minified zg2uni of Rabbit converter
...
function replace_with_rule(rule, output) {
	let max_loop = rule.length;for (let i = 0;i < max_loop;i++) {
		let data = rule[i];let from = data.from;let to = data.to;let from_regex = new RegExp(from, 'g');output = output.replace(from_regex, to);
	}
	return output;
}
