...
Connector.getUniqueID = () => {
	...
	return text && text.split(':').slice(-2).shift();
};
...
Connector.isScrobblingAllowed = () => {
	...
	const categoryLink = Util.getAttrFromSelectors(
		'a[href*="/tags/"]', 'href');

	return pageType && ALLOWED_TYPES.includes(pageType) && categoryLink && 'music' === categoryLink.split('/').pop();
};
...
function getArtistTrackFrom(getter) {
	...
	if (getter === 'videoTitle') {
		...
		if (regPage) {
			artistTrack.artist = regPage[
				PAGE_TITLE_REG_EXP.groups.artist];
		}
