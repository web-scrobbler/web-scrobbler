...
Connector.trackArtSelector = ['.np__avatar_element_image', '.np__player_card_img img'];

Connector.isTrackArtDefault = (url) => {
	return url.includes('/show/images/') || url.includes('/player/') || url.match(/\/artists\/(?!station\/)/);
};

Connector.isPodcast = () => Connector.getTrack().match(/^Program(a)?\s\d+/) !== null;
...
Connector.isPlaying = () => {
	...
	return (
		...
		Util.hasElementClass('.np__btn_controls_play .np__btn_controls_icon', 'fa-pause')
	);
};
